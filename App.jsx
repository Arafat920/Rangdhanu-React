...
function App() {
  ...
  // এপিআই থেকে ডাটা নিয়ে আসা (useEffect)
  useEffect(() => {
    fetchData();
  }, [category]);
  ...
  return (
    <div className="App">
      ...
      {heroMovie && (
        <section className="hero-section" style={{ backgroundImage: `url(${IMG_PATH + heroMovie.backdrop_path})` }}>
          <div className="hero-inner" style={{ zIndex: 10 }}>
            <h1 style={{ fontSize: '40px' }}>{heroMovie.title || heroMovie.name}</h1>
            <p style={{ maxWidth: '600px', margin: '15px 0', opacity: 0.8 }}>{heroMovie.overview.substring(0, 150)}...</p>
